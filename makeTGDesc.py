...
def main_TG():
    ...
    for f in os.listdir(args.resPath):
        if os.path.isdir(args.resPath + "/" + f) or f[-2:] != "TG":
            continue
        tp = f.split(".")[2].upper()
        TF = f.split(".")[4]
        for line in open(args.resPath + "/" + f, "r").readlines():
            ...
            if TG not in symDict:
                outF.write("{}\t{}\t{}\t{}\t{}\t\t\n".format(tp, TF, TFsym, TFdesc, TG))
            else:
                outF.write(
                    "{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                        tp, TF, TFsym, TFdesc, TG, symDict[TG], descDict[TG]
                    )
                )
...
def main():
    ...
    for f in os.listdir(args.resPath + "/TG"):
        if os.path.isdir(args.resPath + "/TG/" + f) or f[-2:] == "TG" or f[-3:] == "txt":
            continue
        print(f)
        tp = "T" + f.split(".")[1]
