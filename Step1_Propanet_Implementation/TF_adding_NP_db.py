...
def TF_adding_NP(DEGli, geneSet, TFli, TFrankFile, DEGnetFile, seed, coverNo=200, coverage=None):
    ...
    Output
        ...
        lst_node_weight (li)
    '''
    DEGnet = nx.read_edgelist(nwkFile,data=(('weight',float),),create_using=nx.DiGraph())
    ...
    for iTF,TF in enumerate(TFli_rank):
        ...
        wk = NP.Walker(nwkFile, absWeight=True)
...
if __name__ == "__main__":    
    parser = argparse.ArgumentParser(usage='python %(prog)s TFliFile nwkFile seedFile DEGfile out n cover') 
...
#    parser.add_argument('-r',help='# repeats for IM')
    parser.add_argument('-p',type=int,help='# process for multiprocessing')
    parser.add_argument('-c',type=float,help='coverage threshold')
    parser.add_argument('-coverNo',type=float)
    ...
    def main_fxn(i):
        ...
        weight_DEG_nwk = weight_tp_cond[weight_tp_cond.index.isin(nwk_tp.nodes())].to_frame()
        ...
        ##preprocess
        tmp=[x for x in nwk_tp.nodes()]
        nodes=pd.Series(list(set(tmp)))
        
        import pdb; pdb.set_trace() # BREAKPOINT
        TFset=set(nodes[nodes.isin(TFli)].tolist())
        ##influence maximization 
        TFrank, infNo = IM(nwk_tp,TFset,50)
        ##Output into file 
        for TF in infNo:
            if infNo[TF]==0:
                TFrank.remove(TF)
        ...
        #with open(args.i+'.TFinf.t'+str(i), 'w') as infF:
        #    for TF in infNo : infF.write(TF+'\t'+str(infNo[TF])+'\n')   
        print >> sys.stderr, 'TF ranking done', time.strftime("%H:%M:%S",time.gmtime(time.time()-start)), '\n'
        ...
        spearmanLi,coverage,TF_trimmed,lst_node_weight = TF_adding_NP(DEGli, geneSet, TFli, TFrankFile, DEGnetFile, seed, 200,coverage=args.c) 
        ##Save Output as file 
        ...
        with open(args.outD+'/log.corr.'+args.cond+'.t'+str(i),'w') as OF2:
            for item in spearmanLi:
                ...
                OF2.flush()
        print >> sys.stderr, 'NP done', time.strftime("%H:%M:%S",time.gmtime(time.time()-start)), '\n'
        ##log
